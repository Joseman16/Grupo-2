...
app.get("/productos", async (request, response)=>{
    try {
        ...
        const responseQuery = await client.query("SELECT pro.pro_id, pro.pro_nombre, cat.cat_id, cat.cat_nombre, pro.pro_precio, pro.pro_stock FROM productos AS pro JOIN categorias AS cat ON pro.cat_id = cat.cat_id;");
...
app.post("/productos", async (request, response) => {
    try {
        ...
        const query = await client.query("INSERT INTO productos(pro_nombre, cat_id, pro_precio, pro_stock) VALUES($1, $2, $3, $4)", [pro_nombre, cat_id, pro_precio, pro_stock]);
        console.log(query);
...
app.post("/ventas", async (request, response) => {
    try {
        ...
        const query = await client.query("INSERT INTO ventas(cli_id, pro_id, ven_cantidad, ven_fecha) VALUES($1, $2, $3, $4)", [cli_id, pro_id, ven_cantidad, ven_fecha]);
        console.log(query);
        response.send(request.body);
        client.release();
    } catch (error) {
